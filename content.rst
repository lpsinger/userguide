...
.. tabs::
  ...
  .. tab:: JSON
    ...
    .. table::
      ...
      +---------------------------------------+-----------------------------------------------------------+-----------------------------------------------------------+
      | search                                | :samp:`{{AllSky, AllSky, BBH, EarlyWarning, MDC}}`                                                                    |
      +---------------------------------------+-----------------------------------------------------------+-----------------------------------------------------------+
  ...
  .. tab:: Avro
    ...
    .. table::
      ...
      +---------------------------------------+-----------------------------------------------------------+-----------------------------------------------------------+
      | search                                | :samp:`{{AllSky, BBH, EarlyWarning, MDC}}`                                                                            |
      +---------------------------------------+-----------------------------------------------------------+-----------------------------------------------------------+
      | group                                 | :samp:`CBC`                                               | :samp:`Burst`                                             |
      +---------------------------------------+-----------------------------------------------------------+-----------------------------------------------------------+
      | pipeline                              | :samp:`{{gstlal,MBTA,pycbc,spiir}}`                       | :samp:`{{CWB,oLIB}}`                                      |
...
+---------------------------------------+-----------------------------------------------------------+-----------------------------------------------------------+
| External Observatory                  | :samp:`{{Fermi,Swift,INTEGRAL,AGILE,SNEWS}}`                                                                          |
+---------------------------------------+-----------------------------------------------------------+-----------------------------------------------------------+
| External Search                       | :samp:`{{GRB,SubGRB,SubGRBTargeted,Supernova}}`                                                                       |
+---------------------------------------+-----------------------------------------------------------+-----------------------------------------------------------+
...
:term:`GWSkyNet` is a machine learning classifier that produces a :term:`GWSkyNet` score for multi-detector :term:`CBC` event candidates. For a multi-detector :term:`CBC` candidate, the values provided on :term:`GraceDB` are the following: 

* **GWSkyNet score**: A measure of :term:`GWSkyNet`’s confidence that an event candidate belongs to the noise or signal class. A :term:`GWSkyNet` score of 0 is consistent with noise, and 1 is consistent with a signal. The score is produced using the sky localization information and the metadata encoded in :term:`FITS` files associated with the event [#GWSkyNet]_. This allows :term:`GWSkyNet` to evaluate an event from a different perspective from the search pipelines, using a common, standardized end product generated by BAYESTAR [#bayestardistance]_ [#bayestar]_. 

* **GWSkyNet False Alarm Probability**: The corresponding :term:`GWSkyNet` :term:`FAP`. The :term:`GWSkyNet` :term:`FAP` measures the fraction of noise events at or above a candidate’s :term:`GWSkyNet` score. This empirical measurement is derived from :term:`GWSkyNet`’s performance on known LIGO-Virgo detector glitches and simulated gravitational wave signals, as described in the :term:`GWSkyNet` output guide [#GWSkyNetoutput]_.
* **GWSkyNet False Negative Probability**: The corresponding :term:`GWSkyNet` :term:`FNP`. The :term:`GWSkyNet` :term:`FNP` measures the fraction of astrophysical events at or below a candidate’s :term:`GWSkyNet` score. This empirical measurement is derived from :term:`GWSkyNet`’s performance on known LIGO-Virgo detector glitches and simulated gravitational wave signals, as described in the :term:`GWSkyNet` output guide [#GWSkyNetoutput]_.

:term:`GWSkyNet` classification is not available for single-detector :term:`CBC` candidates.
...
.. [#GWSkyNetoutput]
   Chan, M. L., McIver, J., Haggard, D., et al. 2023, *GWSkyNet Output Guide*. 
   :dcc:`P2300142`
...
.. [#bayestar]
   Singer, L. P.  and Price, L. R. 2016, |PRD| 93, 024013.
