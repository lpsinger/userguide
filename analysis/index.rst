...
(:term:`FAR`) of :math:`1.62 \times  10^{-4}` Hz (fourteen per day) before considering trials factor. We will divide
...
be sent for all triggers that pass a false alarm rate (FAR) threshold of :math:`2.3 \times  10^{-5}` Hz (two per day).
